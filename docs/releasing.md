...
4. Monitor the Release workflow. It checks the exact tag and runs CI. GoReleaser then builds, signs, and packages without publishing. The workflow scans and saves the packages, submits the macOS executables to Apple, and waits for acceptance. It then checks the same packages on all six native platforms.
